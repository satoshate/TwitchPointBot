...
async def listen_to_eventsub(settings: dict):
    ...
    try:
        ...
        STOP_EVENT.set() # Сигнализируем консоли, что нужно остановиться
...
async def console_input_worker(settings: dict):
    ...
    while not STOP_EVENT.is_set():
        try:
            cmd_line = await asyncio.wait_for(loop.run_in_executor(None, sys.stdin.readline), timeout=1.0)
...
async def main():
    ...
    while True:
        ...
        try:
            # Проверяем, не было ли исключений в завершенной задаче
            for task in done:
                if task.exception():
            ...
            RESTART_FLAG = False # Не перезапускаемся при ошибке токена
