...
def initial_setup(settings):
    ...
    if not settings.get("twitch_oauth_token"):
        print("\n--- GETTING OAuth TOKEN ---")
        print("A special URL will be created for you based on your Client ID.")
        client_id = settings.get("twitch_client_id")
        if not client_id:
            logger.error("Client ID is missing. Please restart and enter it first.")
            return False
        auth_url = (
        ...
        print("\n1. Copy the full URL below.")
        print("2. Paste it into your browser and press Enter.")
        print("3. Authorize your application.")
        print("4. Copy the token from the NEW address bar (after 'access_token=' and before '&').")
        print("\nYOUR URL IS:")
        print(f"\n{auth_url}\n")
        settings["twitch_oauth_token"] = input("Paste your freshly generated OAuth token here: ").strip()
    ...
    try:
        while not STOP_EVENT.is_set():
            ...
            if command == "help":
                ...
                if "key_behavior" not in settings:
                    settings["key_behavior"] = {}
                if key_list_name not in settings["key_behavior"]:
                    settings["key_behavior"][key_list_name] = []
