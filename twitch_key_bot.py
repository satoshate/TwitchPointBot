...
# --- SOUND MANAGEMENT ---
try:
    # Предварительная инициализация для лучшей совместимости
...
def ensure_defaults(settings):
    ...
    # Гарантированно добавляем секцию звука, если ее нет
    if "sound_on_redemption" not in settings:
        settings["sound_on_redemption"] = {
            "enabled": True,
            "sound_file": "sounds/alert.ogg" # Используем OGG для лучшей совместимости
        }
...
async def main():
    ...
    while True:
        ...
        # Гарантированно проверяем и добавляем настройки по умолчанию
        ensure_defaults(settings)
