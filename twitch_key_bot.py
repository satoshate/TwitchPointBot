...
def trigger_sound(sound_file):
    """Stops any currently playing sound and starts a new one using pygame."""
    ...
    try:
        ...
        logger.error(f"Could not play sound with pygame: {e}")
    ...
    try:
        while not STOP_EVENT.is_set():
            ...
            if command == "help":
                ...
                print("  sound <on|off|path>      - Manage redemption sound (e.g., sound on, sound sounds/new.mp3)", flush=True)
...
if __name__ == "__main__":
    try:
        # ### ИСПРАВЛЕНИЕ ###: Запускаем main() через asyncio.run()
        asyncio.run(main())
    except KeyboardInterrupt:
        logger.info("\nScript stopped by user (Ctrl+C).")
    finally:
        # ### ИСПРАВЛЕНИЕ ###: Корректно завершаем работу pygame
